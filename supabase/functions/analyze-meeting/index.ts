...
핵심 작성 지침:
1. **원문 정확성 필수**: 고유명사, 약어, 영문 용어, 제품명, 기술 용어는 원문 그대로 정확히 표기하세요. 예: "RULE8"을 "UL"로, "재작업"을 "제작업"으로 바꾸지 마세요.
2. **오타 주의**: 원본 텍스트의 단어를 임의로 변경하거나 유사 단어로 대체하지 마세요. 확실하지 않으면 원문을 그대로 사용하세요.
3. **시간 정보 제외**: 녹취록의 타임스탬프(예: 46:15-53:42, 00:00 등)는 절대 포함하지 마세요.
4. **간결하게 작성**: 각 항목은 핵심만 담아 2-3문장 이내로 작성하세요.
5. **핵심 수치만 포함**: 중요한 숫자, 금액, 비율만 기재하세요.
6. **주요 논의사항**: 3-5개로 제한하고 각각 간결하게 정리하세요.
