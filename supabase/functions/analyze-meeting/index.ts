...
제공된 회의 자료와 녹취록을 분석하여 **상세하고 포괄적인** 경영진 보고서를 작성해주세요.

중요: 요약보다는 **세부 내용의 정확한 추출**이 우선입니다. 회의에서 논의된 구체적인 내용, 수치, 일정, 담당자 등을 빠짐없이 기록하세요.
...
다음 JSON 형식으로만 응답하세요:
{
  ...
  "summary": "회의 전체 요약 (핵심 결론 위주로 3-5문장, 주요 결정사항과 진행상황 포함)",
  "topics": [
    {
      "title": "주요 논의 사항 제목 (구체적인 카테고리명)",
      "content": "상세 내용을 충분히 작성. 구체적인 수치, 일정, 현황, 계획을 모두 포함. 하위 항목이 있으면 bullet point 형태로 나열. 최소 3-5문장 이상으로 상세히 작성하세요.",
      "subItems": [
        "세부 항목 1: 구체적인 내용과 수치",
        "세부 항목 2: 일정 및 담당자",
        "세부 항목 3: 현재 상태 및 이슈"
...
1. **원문 정확성 필수**: 고유명사, 약어, 영문 용어, 제품명, 기술 용어는 원문 그대로 정확히 표기하세요. 
   ...
   - 예: "F/L Base Cabinet", "TX 모델" 등 제품명 그대로 사용
2. **오타 주의**: 원본 텍스트의 단어를 임의로 변경하거나 유사 단어로 대체하지 마세요.
3. **시간 정보 제외**: 녹취록의 타임스탬프는 절대 포함하지 마세요.
4. **상세하게 작성**: 각 topics의 content는 최소 3-5문장 이상으로 상세히 작성하세요. 요약하지 말고 중요 내용을 모두 포함하세요.
5. **구체적 수치 포함**: 금액, 수량, 일정, 비율 등 모든 구체적 수치를 포함하세요.
6. **주요 논의사항**: 5-8개로 카테고리별로 구분하여 작성하세요.
7. **하위 항목 활용**: subItems를 적극 활용하여 세부 내용을 bullet point로 정리하세요.
8. **참석자 미명시 시**: ["회의 참석자"]로 표시
9. **담당자/기한 미명시 시**: "미정"으로 표시
10. **리스크나 결정사항이 없는 경우**: 빈 배열 []로 표시
11. **회의 목적**: meetingPurpose에 회의의 주요 목적을 3개 이상 구체적으로 작성하세요.`;
